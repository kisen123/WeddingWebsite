...
export function MistletoeDecor() {
  return (
    ...
    >
      ...
      />
      {UP_X.map((x, i) => (
        <UpCluster key={i} x={x} i={i} />
      ))}
      {DOWN_X.map((x, i) => (
        <DownCluster key={i} x={x} i={i} />
      ))}
    </svg>
  );
}
